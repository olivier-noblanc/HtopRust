...
impl WifiManager {
    ...
    pub fn get_wifi_details(&self) -> Option<WifiInfo> {
        use std::ptr;
        ...
        use windows::Win32::NetworkManagement::WiFi::{
            ...
            WlanQueryInterface, dot11_phy_type_erp, dot11_phy_type_he, dot11_phy_type_hrdsss,
            dot11_phy_type_ht, dot11_phy_type_ofdm, dot11_phy_type_vht,
        ...
        unsafe {
            ...
            let result =
                WlanOpenHandle(2, None, &raw mut negotiated_version, &raw mut client_handle);
            ...
            let list = if let Some(l) = interface_list.as_ref() {
                l
            } else {
                WlanCloseHandle(client_handle, None);
                return None;
            ...
            let auth =
                self.translate_auth_algorithm(connection.wlanSecurityAttributes.dot11AuthAlgorithm);
            let cipher = self
                .translate_cipher_algorithm(connection.wlanSecurityAttributes.dot11CipherAlgorithm);
            ...
            let ip_address = if self.demo_mode {
                ...
                UdpSocket::bind("0.0.0.0:0")
                    ...
                    .map_or_else(|_| "N/A".to_string(), |addr| addr.ip().to_string())
            };
    ...
    fn fetch_wlan_auth_info(&self) -> String {
        ...
        unsafe {
            ...
            if EvtNext(query_handle, &mut events_buf, 0, 0, &raw mut returned).is_err()
                || returned == 0
            {
            ...
            for &event_raw in events_buf.iter().take(returned as usize) {
                ...
                if EvtRender(
                    ...
                    1u32,
                    xml_buffer_size,
                    Some(xml_buffer.as_mut_ptr().cast()),
                    &raw mut xml_buffer_used,
                    &raw mut property_count,
                )
                .is_ok()
                {
                    ...
                    if event_id == "12013" {
                        ...
                        let eap_method = match eap_type.as_str() {
                            "13" => "EAP-TLS",
                            "25" => match inner_eap_type.as_str() {
                                "26" => "PEAP-MSCHAPv2",
                                "6" => "PEAP-GTC",
                                "" => "PEAP",
                        ...
                        if !eap_method.is_empty() {
                            auth_info = format!("{}: {eap_method}", t!("wifi.auth_dot1x"));
                            break;
                        }
                    }
                }
    ...
    fn translate_auth_algorithm(
        &self,
        auth: windows::Win32::NetworkManagement::WiFi::DOT11_AUTH_ALGORITHM,
    ) -> String {
        use windows::Win32::NetworkManagement::WiFi::{
            DOT11_AUTH_ALGO_80211_OPEN, DOT11_AUTH_ALGO_80211_SHARED_KEY, DOT11_AUTH_ALGO_IHV_END,
            DOT11_AUTH_ALGO_IHV_START, DOT11_AUTH_ALGO_OWE, DOT11_AUTH_ALGO_RSNA,
            DOT11_AUTH_ALGO_RSNA_PSK, DOT11_AUTH_ALGO_WPA, DOT11_AUTH_ALGO_WPA_NONE,
            DOT11_AUTH_ALGO_WPA_PSK, DOT11_AUTH_ALGO_WPA3, DOT11_AUTH_ALGO_WPA3_SAE,
        };

        match auth {
            DOT11_AUTH_ALGO_80211_OPEN => t!("auth_algo.open").to_string(),
            DOT11_AUTH_ALGO_80211_SHARED_KEY => t!("auth_algo.shared_key").to_string(),
            DOT11_AUTH_ALGO_WPA => t!("auth_algo.wpa").to_string(),
            DOT11_AUTH_ALGO_WPA_PSK => t!("auth_algo.wpa_psk").to_string(),
            DOT11_AUTH_ALGO_WPA_NONE => t!("auth_algo.wpa_none").to_string(),
            DOT11_AUTH_ALGO_RSNA => t!("auth_algo.wpa2").to_string(),
            DOT11_AUTH_ALGO_RSNA_PSK => t!("auth_algo.wpa2_psk").to_string(),
            DOT11_AUTH_ALGO_WPA3 => t!("auth_algo.wpa3").to_string(),
            DOT11_AUTH_ALGO_WPA3_SAE => t!("auth_algo.wpa3_sae").to_string(),
            DOT11_AUTH_ALGO_OWE => t!("auth_algo.owe").to_string(),
            DOT11_AUTH_ALGO_IHV_START | DOT11_AUTH_ALGO_IHV_END => {
                t!("auth_algo.vendor").to_string()
            }
            _ => format!("Unknown({})", auth.0),
        }
    }

    fn translate_cipher_algorithm(
        &self,
        ...
        use windows::Win32::NetworkManagement::WiFi::{
            ...
            DOT11_CIPHER_ALGO_GCMP, DOT11_CIPHER_ALGO_GCMP_256, DOT11_CIPHER_ALGO_IHV_END,
            DOT11_CIPHER_ALGO_IHV_START, DOT11_CIPHER_ALGO_NONE, DOT11_CIPHER_ALGO_TKIP,
            DOT11_CIPHER_ALGO_WEP, DOT11_CIPHER_ALGO_WEP40, DOT11_CIPHER_ALGO_WEP104,
        };

        match cipher {
            DOT11_CIPHER_ALGO_NONE => t!("cipher_algo.none").to_string(),
            DOT11_CIPHER_ALGO_WEP40 => t!("cipher_algo.wep40").to_string(),
            DOT11_CIPHER_ALGO_TKIP => t!("cipher_algo.tkip").to_string(),
            DOT11_CIPHER_ALGO_CCMP => t!("cipher_algo.ccmp").to_string(),
            DOT11_CIPHER_ALGO_WEP104 => t!("cipher_algo.wep104").to_string(),
            DOT11_CIPHER_ALGO_BIP => "BIP".to_string(),
            DOT11_CIPHER_ALGO_GCMP => "AES-GCMP".to_string(),
            DOT11_CIPHER_ALGO_GCMP_256 => "AES-GCMP-256".to_string(),
            DOT11_CIPHER_ALGO_CCMP_256 => "AES-CCMP-256".to_string(),
            DOT11_CIPHER_ALGO_BIP_GMAC_128 => "BIP-GMAC-128".to_string(),
            DOT11_CIPHER_ALGO_BIP_GMAC_256 => "BIP-GMAC-256".to_string(),
            DOT11_CIPHER_ALGO_BIP_CMAC_256 => "BIP-CMAC-256".to_string(),
            DOT11_CIPHER_ALGO_WEP => "WEP".to_string(),
            DOT11_CIPHER_ALGO_IHV_START | DOT11_CIPHER_ALGO_IHV_END => {
                t!("auth_algo.vendor").to_string()
            }
            _ => format!("Unknown({})", cipher.0),
        }
    }
}
