...
use rust_i18n::t;
use system::{CcmStatus, DiskInfo, MemoryInfo, NetworkInfo, ProcessInfo, SortColumn, SystemState};
use theme::Theme;
...
impl App {
    fn new(
        ...
        let host_info = if demo_mode {
            "DEMO-PC".to_string()
        } else {
            System::host_name().unwrap_or("Unknown".to_string())
        };
        let username = if demo_mode {
            "demo_user".to_string()
        } else {
        ...
        match self.view_mode {
            ViewMode::Processes | ViewMode::ProcessDetail => {
                let i = match self.process_table_state.selected() {
                    Some(i) => {
                        if i >= state.processes.len().saturating_sub(1) {
                            0
                        } else {
                            i + 1
                        }
            ...
            ViewMode::SystemEvents | ViewMode::EventDetail => {
                let i = match self.event_table_state.selected() {
                    Some(i) => {
                        if i >= state.events.len().saturating_sub(1) {
                            0
                        } else {
                            i + 1
                        }
        ...
        match self.view_mode {
            ViewMode::Processes | ViewMode::ProcessDetail => {
                let i = match self.process_table_state.selected() {
                    Some(i) => {
                        if i == 0 {
                            state.processes.len().saturating_sub(1)
                        } else {
                            i - 1
                        }
                    }
...
fn spawn_worker(state: Arc<RwLock<AppState>>, rx: mpsc::Receiver<AppAction>, demo_mode: bool) {
    thread::spawn(move || {
        ...
        let _last_tick = Instant::now().checked_sub(Duration::from_secs(60)).unwrap();
            ...
            let processes: Vec<ProcessInfo> = system
                ...
                .map(|(pid, p)| {
                    let user = p
                        ...
                        .map_or_else(|| "N/A".to_string(), |u| u.name().to_string());
...
// Helper to check for interactive console capabilities
fn has_interactive_console_capabilities() -> bool {
    ...
    unsafe {
        let handle_result = GetStdHandle(STD_OUTPUT_HANDLE);
        ...
        // Check if we can enable Virtual Terminal Processing (ANSI sequences)
        if (mode.0 & ENABLE_VIRTUAL_TERMINAL_PROCESSING.0) == 0 {
            // Try to enable it
            let new_mode = CONSOLE_MODE(mode.0 | ENABLE_VIRTUAL_TERMINAL_PROCESSING.0);
            if windows::Win32::System::Console::SetConsoleMode(handle, new_mode).is_err() {
                return false;
            }
        }
...
fn main() -> Result<()> {
    ...
    let plain_arg = args.iter().any(|arg| arg == "--plain");
    ...
    if args.iter().any(|arg| arg == "--help" || arg == "-h") {
        ...
        println!("  --plain         Force plain text mode (recommended for limited remote consoles)");
        println!("  --help, -h      Show this help message");
        return Ok(());
    }

    // Define locale
    let sys_lang = if let Some(l) = sys_locale::get_locale() {
        l
    } else {
        "en".to_string()
    };
    let lang = if sys_lang.to_lowercase().starts_with("fr") {
        "fr"
    } else {
        "en"
    };
    ...
    // Initial check for interactivity logic
    let is_tty = io::stdout().is_terminal();
    let console_capable = has_interactive_console_capabilities();
    let use_plain_mode = plain_arg || !is_tty || !console_capable;

    if !use_plain_mode {
        // Attempt to start Full TUI
        ...
        // Fallback message
        if !plain_arg {
             println!("Running in plain mode (limited console capabilities detected).");
             println!("Use --plain to suppress this reasoning or force this mode.");
             if !is_tty {
                 println!("Reason: Not a TTY");
             } else if !console_capable {
                 println!("Reason: Missing ENABLE_VIRTUAL_TERMINAL_PROCESSING capability");
             }
        }
        
        // Plain mode panic hook
        std::panic::set_hook(Box::new(move |info| {
    ...
    // Setup Backend
    let mut terminal = if use_plain_mode {
         // Plain Mode (Inline)
        ...
        let mut stdout = io::stdout();
        execute!(stdout, crossterm::terminal::EnterAlternateScreen, event::EnableMouseCapture)?;
        let backend = CrosstermBackend::new(stdout);
        Terminal::new(backend)?
    };
    ...
    let res = run_app(&mut terminal, &mut app, use_plain_mode);
...
fn run_app(terminal: &mut Terminal<CrosstermBackend<io::Stdout>>, app: &mut App, is_plain: bool) -> Result<()> {
    loop {
        // 1. Read Shared State
        if is_plain {
            // In "Plain Mode" (Inline TUI), we reset cursor to top-left of our render area
            // to avoid infinite scrolling (simulating a repaint in place)
        ...
        if crossterm::event::poll(timeout).unwrap_or(false) {
            match event::read() {
                Ok(event) => match event {
                        ...
                        if key.kind == event::KeyEventKind::Press {
                            match key.code {
                                ...
                                KeyCode::Char('s') => {
                                    ...
                                    app.message =
                                        format!("Refresh rate: {}s", app.refresh_rate.as_secs());
                                    app.message_time = Some(Instant::now());
                                    if let Some(tx) = &app.action_tx {
                                        let _ =
                                            tx.send(AppAction::SetRefreshRate(app.refresh_rate));
                                    }
                                }
                                ...
                                KeyCode::Char('a' | 'h') => match app.view_mode {
                    ...
                    Event::Mouse(mouse) => match mouse.kind {
                        ...
                        event::MouseEventKind::Down(event::MouseButton::Left) => {
                            if is_plain {
                                // Ignore mouse in plain mode if we want to be strict,
                                // but if we get events, might as well handle them.
                            }
                            ...
                            if mouse.row >= 11
                                && mouse.row <= 17
                                && mouse.column >= col1_end
                                && mouse.column < col2_end
                            {
                            ...
                            if mouse.row > header_height {
                                ...
                                match app.view_mode {
                                    ...
                                    ViewMode::WifiDetails
                                    | ViewMode::EventDetail
                ...
                Err(_) => {
                    // Ignore input errors in weird consoles
                }
            }
        }
...
fn draw_process_detail(f: &mut Frame, app: &App, state: &AppState, area: Rect) {
    ...
    let p = Paragraph::new(detail)
        .block(
            Block::default()
                .borders(Borders::ALL)
                .title(t!("processes.detail_title"))
                .style(
                    Style::default()
                        .bg(app.theme.background)
...
fn draw_header(f: &mut Frame, app: &App, state: &AppState, area: Rect) {
    ...
    f.render_widget(
        Paragraph::new(t!("header.cpu")).style(
            Style::default()
                .bg(app.theme.background)
                .fg(app.theme.highlight)
    ...
    f.render_widget(
        Paragraph::new(format!("{}: {}", t!("header.uptime"), uptime_str))
            .style(Style::default().bg(app.theme.background).fg(app.theme.text)),
        mid_chunks[2],
    );

    f.render_widget(
        Paragraph::new(format!("OS: {}", app.os_info))
            .style(Style::default().bg(app.theme.background).fg(app.theme.text)),
        mid_chunks[3],
    );
    ...
    let privilege_color = if app.is_admin {
        app.theme.warning
    } else {
        app.theme.text
    };
    ...
    f.render_widget(
        Paragraph::new(format!(
            "Host: {} | User: {}{}",
            app.host_info, app.username, privilege_badge
        ))
        .style(
            Style::default()
                .bg(app.theme.background)
    ...
    if let Some(ccm) = &state.ccm_status {
        let status_color = if ccm.running {
            app.theme.success
        } else {
            app.theme.error
        };
        ...
        let info = format!(
            "CCM: {} | Actions: {}{}",
            ccm.status, ccm.pending_actions, error_text
        );
        f.render_widget(
            Paragraph::new(info).style(Style::default().bg(app.theme.background).fg(status_color)),
            mid_chunks[5],
        );
    } else {
        f.render_widget(
    ...
    f.render_widget(
        Paragraph::new(t!("header.disks")).style(
            Style::default()
                .bg(app.theme.background)
                .fg(app.theme.warning)
                .add_modifier(Modifier::BOLD),
    ...
    f.render_widget(
        Paragraph::new(event_title).style(
            Style::default()
                .bg(app.theme.background)
                .fg(app.theme.error)
                .add_modifier(Modifier::BOLD),
    ...
    for err in state.events.iter().take(2) {
        let truncated = if err.message.len() > 50 {
    ...
    if err_text.is_empty() {
        err_text = t!("events.no_events").to_string();
    }
    f.render_widget(
        Paragraph::new(err_text).style(Style::default().bg(app.theme.background).fg(Color::Gray)),
        mid_chunks[10],
    );
    ...
    f.render_widget(
        Paragraph::new(t!("header.network")).style(
            Style::default()
                .bg(app.theme.background)
                .fg(app.theme.network)
                .add_modifier(Modifier::BOLD),
        ),
        net_chunks[0],
    );
    ...
    for net in &state.system_state.networks {
        if net.rx > 0 || net.tx > 0 {
            net_info.push_str(&format!(
                "{}: R:{:.1}K T:{:.1}K\n",
                net.name,
                net.rx as f32 / 1024.0,
                net.tx as f32 / 1024.0
            ));
        }
    }
    f.render_widget(
        Paragraph::new(net_info)
            .style(Style::default().bg(app.theme.background).fg(app.theme.text)),
        net_chunks[1],
    );
}
...
fn draw_process_table(f: &mut Frame, app: &mut App, state: &AppState, area: Rect) {
    ...
    let header_row = Row::new(
        headers
            .iter()
            .map(|h| Cell::from(*h).style(Style::default().fg(app.theme.highlight))),
    )
    .height(1)
    .bottom_margin(1);
...
fn draw_event_table(f: &mut Frame, app: &mut App, state: &AppState, area: Rect) {
    let header_cells = ["TIME", "SOURCE", "ID", "MESSAGE"]
        .iter()
        .map(|h| Cell::from(*h).style(Style::default().fg(app.theme.highlight)));
...
fn draw_footer(f: &mut Frame, app: &App, area: Rect) {
    ...
    let p = Paragraph::new(help).style(
        Style::default()
